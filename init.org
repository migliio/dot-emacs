...
#+begin_src emacs-lisp
  ...
  (use-package action-on-region-str
    :ensure nil
    :bind
    (("C-c f k s" . replace-string)
     ("C-c f k c" . comment-region)
     ("C-c f k u" . uncomment-region)
     ("C-c f k i" . indent-region)))
    ...
    (setq org-capture-templates
	  '(("f" "File" plain (file (lambda() (mg/create-pages-file)))
	    ...
	    ("ae" "Call entry" entry (file+headline "~/Vault/pkm/pages/agenda.org" "Future")
	     "* Call with %? :call:\n:PROPERTIES:\n:CATEGORY: %^{Category}\n:END:\n:LOGBOOK:\n- Entry inserted on %U \\\\\n:END:\n:PROPERTIES:\n:NOTIFY_BEFORE:\n:END:\n%^T\n")
	    ("F" "Flashcards")
	    ("Fm" "Mathematics flashcard" entry (file+headline "~/Vault/pkm/pages/flashcards.org" "Mathematics")
	     "* %^{Question} :drill:\n[%?]")
	    ("Fk" "Kernel flashcard" entry (file+headline "~/Vault/pkm/pages/flashcards.org" "Kernel")
	     "* %^{Question} :drill:\n[%?]")
	    ("r" "Resources")
...
#+begin_src emacs-lisp
  (use-package mg-pkm
    ...
    (defun mg/search-store-link-place-and-paste ()
      "Search an heading in contacts file, do org-store-link and then yank the link"
      (interactive)
      (let ((contacts-file "~/Vault/pkm/pages/places.org"))
	(mg/search-store-link-heading-and-paste contacts-file)))
