  ...
  (if (display-graphic-p)
      (progn
  	  ...
  	  (progn
  	    ...
  	    (set-frame-font "Noto Sans Mono 17" nil t))
  	(progn
...
#+begin_src emacs-lisp
  ...
  (defun mg/commit-assets ()
    "Commit assets, both in pkm and zotero-related, automatically"
    (interactive)
    (let ((default-directory "~/Zotero/storage"))
      (shell-command "git add . && git commit -m \"commit\"")))
