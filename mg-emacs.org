...
The following piece of code introduces ~hide-mode-line~ mode, which is used in a custom function of mine to enable PDF presentation mode (not technically an Emacs mode, rather a custom layout). Moreover, I have some customization on the mode-line colors, which is perfect in conjunction with the default Emacs dark theme.
